...
from envs import GlobalContinusNav2
from stable_baselines3.common.logger import configure

tmp_path = "./simple5-world"
# new_logger = configure(tmp_path, ['stdout','csv','tensorboard'])

env = GlobalContinusNav2()
# env.eval_model()
...
episode_return = 0
spl = 0
while env.simulation_times() < test_times:
    action, _states = model.predict(obs)
    obs, rewards, dones, info = env.step(action)
    episode_return += rewards
    if dones:
        if rewards > 10 and info["global_done"]:
...
print(
    f"success times:{success_times}, simulation times:{test_times}, success rate[SR]:{success_times / test_times}, SPL:{spl / test_times}"
)
