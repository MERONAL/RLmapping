...
class NavigationEnv:
    def __init__(self, env, symbolic, seed, max_episode_length, action_repeat, bit_depth):
        from rl.scripts.envs.env.local_env.local_nav_env import DreamerEnv
        train_config = "rl/configs/train_config.yaml"
        self.symbolic = symbolic
        self._env = DreamerEnv(train_config=train_config, init_target_distance=2, episode_maximum_steps=512)
        # self._env.seed(seed)
...
def Env(env, symbolic, seed, max_episode_length, action_repeat, bit_depth):
    if env in GYM_ENVS:
        ...
        return NavigationEnv(env,symbolic, seed, max_episode_length, action_repeat, bit_depth)
...
if __name__ == '__main__':
    env = Env('navigation', symbolic=True,seed=0,max_episode_length=1024, action_repeat=2, bit_depth=1)
    print(env.observation_size) # 3
    print(env.action_size) # 1
    obs = env.reset() # 
    print(obs.shape) #torch.Size([1, 3])
    steps = 0
    while True:
        obs, reward,done= env.step(env.sample_random_action())
        print(obs) 
        print(reward)
        print(done)
        steps += 1
