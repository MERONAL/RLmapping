...
from rl.scripts.maxent_dreamer.env import Env
from rl.scripts.maxent_dreamer.memory import ExperienceReplay
from rl.scripts.maxent_dreamer.models import bottle, Encoder, ObservationModel, RewardModel, TransitionModel, ActorSAC, CriticTwin
from planner import MPCPlanner
from rl.scripts.maxent_dreamer.utils import lineplot, write_video, imagine_ahead, lambda_return, FreezeParameters
...
parser.add_argument('--env', type=str, default='navigation',
                    help='Gym/Control Suite environment')
parser.add_argument('--symbolic-env', action='store_true', help='Symbolic features')
parser.add_argument('--max-episode-length', type=int, default=2000, metavar='T', help='Max episode length')
...
parser.add_argument('--embedding-size', type=int, default=128, metavar='E',
...
parser.add_argument('--belief-size', type=int, default=64, metavar='H', help='Belief/hidden size')
parser.add_argument('--state-size', type=int, default=30, metavar='Z', help='State/latent size')
parser.add_argument('--action-repeat', type=int, default=2, metavar='R', help='Action repeat')
parser.add_argument('--action-noise', type=float, default=0.3, metavar='ε', help='Action noise')
parser.add_argument('--episodes', type=int, default=5000, metavar='E', help='Total number of episodes')
...
parser.add_argument('--chunk-size', type=int, default=128, metavar='L', help='Chunk size')
...
# Setup
results_dir = os.path.join('/home/mhy/exp/rl/local_avoidance_mb/results', '{}_{}'.format(args.env, args.id))
...
metrics = {'steps': [], 'episodes': [], 'train_rewards': [], 'test_episodes': [], 'test_rewards': [],
           'observation_loss': [], 'reward_loss': [], 'kl_loss': [], 'actor_loss': [], 'value_loss': [], 'alpha':[]} #TODO:check
...
if args.experience_replay is not '' and os.path.exists(args.experience_replay):
    ...
    metrics['steps'], metrics['episodes'] = D['steps'], D['episodes']
elif not args.test:
...
actor_model = ActorSAC(args.belief_size, args.state_size, args.hidden_size, env.action_size).to(device=args.device)
target_actor_model =  ActorSAC(args.belief_size, args.state_size, args.hidden_size, env.action_size).to(device=args.device)

value_model = CriticTwin(args.belief_size, args.state_size, env.action_size, args.hidden_size).to(device=args.device)
target_value_model = CriticTwin(args.belief_size, args.state_size, env.action_size, args.hidden_size).to(device=args.device)
alpha_log = torch.tensor((-np.log(env.action_size) * np.e,), dtype=torch.float32, requires_grad=True, device=args.device)
obj_critic = (-np.log(0.5)) ** 0.5
target_entropy = np.log(env.action_size)
...
value_optimizer = optim.Adam(value_model.parameters(),
                             ...
                             eps=args.adam_epsilon)
#
alpha_optim = torch.optim.Adam((alpha_log,), lr=0 if args.learning_rate_schedule != 0 else args.actor_learning_rate)

if args.models is not '' and os.path.exists(args.models):
    model_dicts = torch.load(args.models, map_location=args.device)
    transition_model.load_state_dict(model_dicts['transition_model'])
    observation_model.load_state_dict(model_dicts['observation_model'])
    reward_model.load_state_dict(model_dicts['reward_model'])
    encoder.load_state_dict(model_dicts['encoder'])
    actor_model.load_state_dict(model_dicts['actor_model'])
    target_actor_model.load_state_dict(model_dicts['actor_model'])
    value_model.load_state_dict(model_dicts['value_model'])
    target_value_model.load_state_dict(model_dicts['value_model'])
...
def update_belief_and_act(args, env, planner, transition_model, encoder, belief, posterior_state, action, observation,
    ...
    if args.algo == "dreamer":
        if not if_test: action = planner.get_action(belief, posterior_state, det=not (explore))
        else: action = planner(belief, posterior_state)
        action = action.detach() #todo:check
    else:
        ...
        for _ in tqdm(range(args.test_episodes)):
            ...
            for t in pbar:
                belief, posterior_state, action, observation, reward, done = update_belief_and_act(args, env, planner,
                                                                                                   ...
                                                                                                   action,
                                                                                                   observation.to(
                                                                                                     device=args.device),if_test=True)
                total_reward += reward
                if args.render:
                    env.render()
                if done:
...
for episode in tqdm(range(metrics['episodes'][-1] + 1, args.episodes + 1), total=args.episodes,
    ...
    for s in tqdm(range(args.collect_interval)):
        ...
        # transition loss # posterior:Q(func)
        ...
        alpha = alpha_log.exp()
        with torch.no_grad():
            next_a, next_log_prob = bottle(target_actor_model.get_action_logprob, (imged_beliefs, imged_prior_states))
            value_pred = torch.min(*bottle(target_value_model, (imged_beliefs, imged_prior_states, next_a)))
        #mhy
        returns = lambda_return(imged_reward, value_pred, bootstrap=value_pred[-1], discount=args.discount,
                                lambda_=args.disclam, next_log_prob=next_log_prob, alpha=alpha) # H step V lambda
        ...
        with torch.no_grad():
            ...
            imged_actions_value = imged_actions.detach()
        ...
        value_pred_q1, value_pred_q2 = bottle(value_model, (value_beliefs, value_prior_states, imged_actions_value))

        value_loss = F.mse_loss(value_pred_q1, target_return) + F.mse_loss(value_pred_q2, target_return)
        ...
        soft_update(target_value_model, value_model, soft_update_tau)

        action_pg, logprob = bottle(actor_model.get_action_logprob, (value_beliefs, value_prior_states))
        obj_alpha = (alpha_log * (logprob - target_entropy).detach()).mean()
        alpha_optim.zero_grad()
        obj_alpha.backward()
        alpha_optim.step()
        soft_update(target_actor_model, actor_model, soft_update_tau)

        with torch.no_grad():
            alpha_log[:]= alpha_log.clamp(-16, 2).detach()

        actor_loss = -torch.mean(torch.min(*bottle(value_model, (value_beliefs, value_prior_states, action_pg))) + logprob * alpha)
        actor_optimizer.zero_grad()
        actor_loss.backward()
        nn.utils.clip_grad_norm_(actor_model.parameters(), args.grad_clip_norm, norm_type=2)
        actor_optimizer.step()
        ...
        losses.append(
            [observation_loss.item(), reward_loss.item(), kl_loss.item(), actor_loss.item(), value_loss.item(), alpha.item()]) #TODO
    ...
    lineplot(metrics['episodes'][-len(metrics['alpha']):], metrics['alpha'], 'alpha', results_dir)
        ...
        for _ in range(10):
            with torch.no_grad():
                ...
                while True:
                    ...
                    observation = next_observation
                    if done:
                        test_result.append(total_rewards)
                        break

        test_result = np.array(test_result).mean()
        # Update and plot reward metrics (and write video if applicable) and save metrics
        metrics['test_episodes'].append(episode)
        metrics['test_rewards'].append(test_result)
    ...
    writer.add_scalar("alpha", metrics['alpha'][0][-1], metrics['steps'][-1])
