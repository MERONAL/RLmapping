...
def imagine_ahead(prev_state, prev_belief, policy, transition_model, planning_horizon):
    '''
    ...
    T = planning_horizon
    beliefs, prior_states, prior_means, prior_std_devs, actions_list = [torch.empty(0)] * T, [torch.empty(0)] * T, [
    ...
    for t in range(T - 1):
        ...
        actions_list[t] = actions

        steps += 1

    imagined_traj = [torch.stack(beliefs[1:steps], dim=0), torch.stack(prior_states[1:steps], dim=0),
                     torch.stack(prior_means[1:steps], dim=0), torch.stack(prior_std_devs[1:steps], dim=0), torch.stack(actions_list[1:steps], dim=0)]
    return imagined_traj


def lambda_return(imged_reward, value_pred, bootstrap, next_log_prob, alpha, discount=0.99, lambda_=0.95):
    # Setting lambda=1 gives a discounted Monte Carlo return.
    # Setting lambda=0 gives a fixed 1-step return.

    next_values = torch.cat([value_pred[1:], bootstrap[None]], 0)  + alpha * next_log_prob
